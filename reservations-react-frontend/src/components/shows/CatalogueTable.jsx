...
const CatalogueTable = ({ shows = [] }) => {
  ...
  return <div className="catalogue-table-wrap" tabIndex="0" aria-label="Show catalogue table">
    <table className="catalogue-table">
      <thead><tr><th scope="col">Title</th><th scope="col">Venue</th><th scope="col">Next performance</th><th scope="col">Price</th><th scope="col">Availability</th><th scope="col"><span className="sr-only">Open show</span></th></tr></thead>
      <tbody>{shows.map((show) => <tr key={show.id}>
        ...
        <td>{show.nextPerformanceDate ? `${formatDate(show.nextPerformanceDate)}${show.nextPerformanceTime ? ` · ${String(show.nextPerformanceTime).slice(0, 5)}` : ""}` : "To be announced"}</td>
